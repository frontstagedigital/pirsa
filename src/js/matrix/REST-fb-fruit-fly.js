...
// No facets are hardcoded, always fetch latest facets from FB - truly dynamic version.
// 
// PROGRAM FLOW:
...
function getSpellCheckHTML(spellSuggestion, currentQuery) {
  ...
  return '<li>Did you mean <a href="' + spellUrl + '">' + spellSuggestion + '</a></li>';
}
...
function processFacet(facet, facetMap, data) {
  ...
  // Skip facets that are configured to be hidden
  var hiddenFacets = [
    'Document type',
    'Page type'
  ];

  if (hiddenFacets.indexOf(facetName) !== -1) {
    return null;
  }
...
function calculateLayoutConfig(searchData, apiData) {
  ...
  var hasRelatedItems = currentQuery && currentQuery !== '!null' &&
    currentTab !== 'research' && currentTab !== 'pirsa~ds-contacts' &&
...
function generateResultItem(result) {
  ...
  html.add('</div>'); // Close nsw-list-item__content__wrapper
...
function analyzeContentType(result) {
  ...
  var pageType = safeGet(result, 'listMetadata.pageType.0', '');

  return {
    isEvent: collection === 'pirsa~ds-news-events' && newsType === 'Event',
    isNews: collection === 'pirsa~ds-news-events' && newsType === 'News',
    isWebsitePage: collection === 'pirsa~ds-website' && pageType === 'page',
    isWebsiteNews: collection === 'pirsa~ds-website' && pageType === 'news',
...
function generateResultMeta(contentType, result) {
  ...
  if (contentType.isContact) {
    html.add('<div class="nsw-list-item__label">Contact</div>');
  } else if (contentType.isEvent || contentType.isNews) {
    var label = contentType.isEvent ? 'Event' : 'News Release';
    html.add('<div class="nsw-list-item__label">' + label + '</div>');
    if (result.date) {
      var eventDate = new Date(result.date);
      var formattedDate = eventDate.toLocaleDateString('en-AU', {
        day: 'numeric', month: 'long', year: 'numeric'
      });
      html.add('<div class="nsw-list-item__info-container nsw-display-flex nsw-p-top-xs nsw-justify-content-between">');
      html.add('<div class="nsw-list-item__info">' + formattedDate + '</div>');
      html.add('</div>');
    }
  } else if (contentType.isResearch) {
    html.add('<div class="nsw-list-item__label">Research</div>');
  } else if (contentType.isNews) {
    html.add('<div class="nsw-list-item__label">News Release</div>');
  } else if (contentType.isWebsitePage) {
    html.add('<div class="nsw-list-item__label">Page</div>');
  } else if (contentType.isWebsiteNews) {
    html.add('<div class="nsw-list-item__label">News</div>');
  } else if (contentType.isReport) {
    html.add('<div class="nsw-list-item__label">Report</div>');
  } else if (!contentType.isDocument) {
    html.add('<div class="nsw-list-item__label">Page</div>');
  }
...
function generateResultTitle(contentType, result) {
  ...
  if (contentType.isContact) {
    var contactName = safeGet(result, 'listMetadata.title.0', result.title || 'Untitled');
    // Remove "FAB " prefix from contact names
    contactName = contactName.replace(/^FAB /, '');
    html.add('<span style="text-decoration:none;">' + contactName + '</span>');
  } else {
...
function generateResultSummary(contentType, result) {
  if (contentType.isContact) {
    return generateContactSummary(result);
  } else if (result.summary) {
    return '<div class="nsw-list-item__copy">' + result.summary + '</div>';
  }
  return '';
}

function generateContactSummary(result) {
  var html = createHtmlBuffer();
  var contactTitle = safeGet(result, 'listMetadata.contactTitle.0', '');
  var contactEmail = safeGet(result, 'listMetadata.contactEmail.0', '');
  var contactPhone = safeGet(result, 'listMetadata.contactPhone.0', '');

  html.add('<div class="nsw-list-item__copy">');

  if (contactTitle) html.add(contactTitle + '<br>');
  if (contactEmail) html.add('<a href="mailto:' + contactEmail + '">' + contactEmail + '</a><br>');
  if (contactPhone) html.add('<a href="tel:' + contactPhone + '">' + contactPhone + '</a>');
