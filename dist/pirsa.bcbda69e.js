 ...
 */ (function($) {
    ...
    /* Public methods */ autocompletion.prototype.init = function() {
    ...
    autocompletion.prototype.option = function(key, val) {
        ...
        if ($.isString(key)) {
            if (arguments.length === 1 || !$.isDefinied(val)) return $.dataVals($.extend({}, that.options), key);
            options[key] = val;
        }
    ...
    autocompletion.prototype.initTypeahead = function() {
        ...
        if (that.options.typeahead.events) $.each(that.options.typeahead.events, function(eventName, func) {
    ...
    /* Private variables */ var _debug = false, _mapKeys = [
        'collection',
        'callback',
        'dataType',
        'alpha',
    ...
    /* Private methods */ // Check if there is enough data to trigger auto-completion
    ...
    function _getSetData(set, name) {
        ...
        function getBloodhoundRemote() {
            ...
            if (set.dataType === 'jsonp') remote['prepare'] = function(query, settings) {
                settings.dataType = 'jsonp';
                settings.url = settings.url.replace(set.queryVal, query);
                return settings;
            };
            else remote['wildcard'] = set.queryVal;
            return remote;
        }
        ...
        function source(query, sync, async) {
            if (query.length < 1 && set.defaultCall) {
                if ($.isString(set.defaultCall)) query = set.defaultCall;
                    ...
                    return;
                } else if ($.exist(set.defaultCall.url, true)) $.get(set.defaultCall.url, set.defaultCall.params, function(data) {
    ...
    function _groupSetData(set, results) {
        ...
        if ($.exist(set.groupOrder)) for(i = 0, len = set.groupOrder.length; i < len; i++)grouped[set.groupOrder[i]] = [
            {
                label: set.groupOrder[i]
            }
        ];
        for(i = 0, len = results.length; i < len; i++){
            if (!$.exist(grouped[results[i][set.itemGroup]])) grouped[results[i][set.itemGroup]] = [
                {
    ...
    function _selectItem(item, target) {
        if ($.exist(item.extra)) switch(item.extra.action_t){
            case 'C':
    ...
    /* Handle Typeahead navigation */ // Navigate dropdown list  left - right (switching between columns)
    function _navCursorLR(code, cols, target) {
        if (!$.exist(_navCols.cursor)) return;
        var currCol = _navCols.cursor.parent(), currColIdx = cols.index(currCol), delta = code == 37 ? -1 : 1, nextColItems = getNextColItems(currColIdx), cursorIdx = $(currCol).children('.tt-selectable').index(_navCols.cursor), nextCursor = $.exist(nextColItems[cursorIdx]) ? nextColItems[cursorIdx] : nextColItems[nextColItems.length - 1];
        ...
        function getNextColItems(currColIdx) {
            var nextColIdx = code == 37 ? $.exist(cols[currColIdx - 1]) ? currColIdx - 1 : cols.length - 1 : $.exist(cols[currColIdx + 1]) ? currColIdx + 1 : 0, nextColItems = $(cols[nextColIdx]).children('.tt-selectable');
    ...
    function _navCursorUD(code, menu, target) {
        ...
        var currCol = _navCols.cursor.parent(), currColItems = $(currCol).children('.tt-selectable');
    ...
    function Plugin() {
        ...
        return this.each(function() {
            var $this = $(this), data = $this.data('flb.autocompletion'), options = $.extend(true, {}, autocompletion.defaults, data || {}, $.isObject(option) && option);
    ...
    $.exist = function(obj, bString) {
        if (!$.isDefinied(bString)) bString = false;
        var obj = bString ? obj : $(obj);
        return $.isDefinied(obj) && obj != null && ($.isString(obj) ? obj + '' : obj).length > 0;
    };
    ...
    $.dataKeys = function(obj) {
        ...
        function iterateKeys(obj, prefix) {
            return $.map(Object.keys(obj), function(key) {
                if (obj[key] && $.isObject(obj[key])) return iterateKeys(obj[key], key);
                else return prefix ? prefix + '-' + key : key;
            });
        }
    };
    $.dataVals = function(obj, key) {
        var parts = key.split('.'), key = parts.shift();
        if (parts.length) for(var i = 0, len = parts.length; i < len; i++){
            obj = obj[key] || {};
            key = parts[i];
        }
        return obj[key];
...
//# sourceMappingURL=pirsa.bcbda69e.js.map
